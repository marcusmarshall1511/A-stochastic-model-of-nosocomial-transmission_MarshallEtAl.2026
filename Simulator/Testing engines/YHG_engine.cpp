...
// [[Rcpp::export]]
NumericVector fast_trial_cpp_HG(int nmaxmax, double gamma, double rC,
    ...
    //               for iHQ in q:iHQ_init    [q to iHQ_init]
    ...
    for (int nmax = 0; nmax <= nmaxmax; nmax++) {
        ...
        for (int n = 0; n <= nmax; n++) { // increasing n
            for (int l_ = 0; l_ <= l_init; l_++) { // decreasing l
                int g = g_init + nmax - n;
                if (g > NHG) continue; // ensures exposed+infectious dont exceed NHG

                // iHG ranges from g down to iHG_init
                for (int iHG = g; iHG >= iHG_init; iHG--) { //given g, start with no exposed HG (all infectious)
                    int eHG = g - iHG;

                    // q ranges from NHQ to q_init
                    for (int q = NHQ; q >= q_init; q--) { 
                                    ...
                                    if (n == 0) {
                                        ...
                                        if (SHG > 0) {
                                            rhs += lambda_HG * get_val(l_, eHQ, iHQ,
                                                                        eHG+1, iHG, eP, iP, n - 1); //HG exposure
                                        }  
                                    }
                                    if (SHQ > 0) {
                                            rhs += lambda_HQ * get_val(l_, eHQ + 1, iHQ,
                                                                        eHG, iHG, eP, iP, n); //HQ exposure
                                        }
                                    if (l_ > 0) {
