...
for (str in strs) {
  ...
  if (str=="0d90e0d10"){
    breakss<- c(breaks,0.95)
    breaksm <- c(breaks)
  } else if (str=="0d90e1d00"){
    breakss<- c(breaks,1.05)
    breaksm <- c(breaks,0.75,0.78)
  } else if (str=="0d75e0d25"){
    breakss<- c(breaks,0.64)
    breaksm <- c(breaks,0.35,0.38)
  } else if (str=="0d75e1d00"){
    breakss<- c(breaks,0.65,0.69)
    breaksm <- c(breaks,0.45,0.49)
  } else if (str=="0d50e0d50"){
    # breakss<- c(breaks,0.65,0.69)
    breaksm <- c(breaks,0.13,0.17)
  } else if (str=="0d50e1d00"){
    breakss<- c(breaks,0.32,0.34)
    breaksm <- c(breaks,0.23)
  } 
  ...
  NS_plot <- ggplot(pl[pl$Category==str,], aes(x=R, y=1/gamma, fill=NS)) +
    ...
    geom_text_contour(aes(z = NS),
                      ...
                      breaks=breakss,
                      skip=1) +
    scale_fill_gradientn(colours=c("white", "#fee090", "#f46d43", "#67001F"),
                         limits=range_diff,
                         values=scales::rescale(c(0, max(abs(range_diff))/10, max(abs(range_diff))/2, max(range_diff))),
                         oob=scales::squish) +
    scale_y_continuous(breaks=c(1/3, 1:7),
                       labels=expression(1/3, 1,2,3,4,5,6,7)) +
